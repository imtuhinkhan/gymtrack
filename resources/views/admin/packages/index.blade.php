...
<div class="min-h-screen overflow-x-hidden">
    ...
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-gray-800">Packages Management</h1>
        @can('create_packages')
                                ...
                                <tr>
                                    ...
                                    <td class="px-3 py-4 text-right text-sm font-medium">
                                        <div class="flex justify-end space-x-2">
                                            <a href="{{ route('admin.packages.show', $package->id) }}" class="btn btn-sm btn-secondary">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            @endcan
                                            @can('edit_packages')
                                            <form method="POST" action="{{ route('admin.packages.toggle-status', $package->id) }}" class="inline">
                                                @csrf
                                                <button type="submit" class="btn btn-sm btn-{{ $package->is_active ? 'warning' : 'success' }}">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            @endcan
                                            @can('create_packages')
                                            <form method="POST" action="{{ route('admin.packages.duplicate', $package->id) }}" class="inline">
                                                @csrf
                                            ...
                                            @endcan
                                            @can('delete_packages')
                                            <form method="POST" action="{{ route('admin.packages.destroy', $package->id) }}" class="inline" onsubmit="return confirm('Are you sure you want to delete this package?')">
                                                @csrf
                                                @method('DELETE')
